# Network Design Project – Team NHT
...
|---|---|---|
| Nathan Hoang | Nathan_Hoang@student.uml.edu |  |
|  |  |  |
|  |  |  |
...
Receiver:
```bash
python "Phase1(b)_receiver.py" --port 9000 --seed 0
```
Sender:
```bash
python3 "Phase1(a)_sender.py" --host 127.0.0.1 --port 9000
```
Expected behavior:Hello gets echoed back to the user
- ...

### Scenario 2: input.bmp
Receiver:
```bash
python3 "Phase1(b)_receiver.py" --port 9001 --out received.bmp
```
Sender:
```bash
python3 "Phase1(b)_sender.py" --host 127.0.0.1 --port 9001 --file test.bmp
```
---
